...
def nearest_nodes(start,end_nodes, arena_parameters, orientation):
	paths_lens=[]
	paths=[]
	orientations = []
	for i in end_nodes:
		...
		# print(back_path)
		moves, new_orientation = pb_theme.paths_to_moves(back_path, arena_parameters["traffic_signals"], orientation)
		# print(moves)
		path_len = len(moves)
	...
	return list(end_nodes)[index] , paths[index], orientations[index]
...
def task_6_implementation(sim,arena_parameters):
	...
	for count in range(counter):	
		...
		for i in range(3):
			if len(medicine_packages)==0:
				pb_theme.send_message_via_socket(connection_2,'b')
				# print(".")
				break
		...
		for i in range(len(deli)):
			...
			node , moves, orientation=nearest_nodes(start, deli,  arena_parameters, orientation)
			path = ''
			for move in moves:
				path = path + move+','
			pb_theme.send_message_via_socket(connection_2, path)
	...
	# Going to end node
	end_node = arena_parameters['end_node']
	node , moves, orientation=nearest_nodes(start, [end_node],  arena_parameters, orientation)
	path = ''
	for move in moves:
		path = path + move+','
	pb_theme.send_message_via_socket(connection_2, path)
