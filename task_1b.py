...
def detect_ArUco_details(image):
    ...
    bbox, ids, r = aruco.detectMarkers(thresh, arucoDict)

    # print(r)
    
    for i in range(len(ids)):
        
        x_sum = bbox[i][0][0][0]+ bbox[i][0][1][0]+ bbox[i][0][2][0]+ bbox[i][0][3][0]
        y_sum = bbox[i][0][0][1]+ bbox[i][0][1][1]+ bbox[i][0][2][1]+ bbox[i][0][3][1]
            
        x_centerPixel = int(x_sum*0.25) 
        y_centerPixel = int(y_sum*0.25)

        top_center_x = (bbox[i][0][0][0]+bbox[i][0][1][0])/2
        top_center_y = (bbox[i][0][0][1]+bbox[i][0][1][1])/2


        # print('X: ',x_centerPixel, top_center_x)
        # print('Y: ',y_centerPixel, top_center_y)

        # x1 = bbox[i][0][0][0] - bbox[i][0][3][0]
        # y1 = bbox[i][0][0][1] - bbox[i][0][3][1]

        x1 = x_centerPixel - top_center_x
        y1 = y_centerPixel - top_center_y

        angle = int(math.degrees((math.atan2(x1,y1))))
        # if x1 > 0 and y1>0 :
        #     angle -= 180
        # elif x1>0 and y1 <0:
        #     angle= angle
        ...
        # print((bbox[i][0][0][0]+bbox[i][0][1][0])/2 , x_centerPixel)

        # if (bbox[i][0][0][0]+bbox[i][0][1][0])/2 < x_centerPixel:
        #     angle= 180 +angle
        # else:
        #     angle= angle - 180

        
        # print(bbox[i][0][1], bbox[i][0][3])   


        

        ArUco_details_dict[int(ids[i][0])] = [[x_centerPixel,y_centerPixel],angle]
        
        ArUco_corners[int(ids[i][0])] = bbox[i][0].astype('int32')
